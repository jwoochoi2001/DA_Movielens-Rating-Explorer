...
### ③ 대시보드
실시간 추천 — 장르·TF-IDF 줄거리 기반 개인화, 사용자 기반·아이템 기반·행렬분해 협업 필터링,
감독·출연진 기반, 비개인화(장르·인기도) 추천까지 전부 포함. 8장 참고.
...
- **행렬분해(8-12) k=2는 표현력이 낮다.** `outputs/tables/mf_recall_by_user.csv` 로 무작위
  사용자 20명 Recall@10을 재보면 0%(정답이 상위 10위 안에 전혀 안 든다) — 추천 자체는 정상
  작동하지만(사용자마다 다른, 준수한 영화를 추천), 후보가 수천 편인 데이터셋에서 k=2·정규화
  없는 SGD로는 세밀한 순위를 가리기 어렵다. k를 10~20으로 올리면 학습 RMSE는 뚜렷이 낮아진다
  (`outputs/tables/mf_grid_train_rmse.csv`).
- **아이템 기반 CF(8-11)의 공통 평가자 수 하한을 엄격히 걸면(예: 30명) 후보가 급감**한다 —
  사용자 414 기준 전체 영화 대비 약 1% 수준(46편)만 남을 때도 있었다. 최소 평가 수 필터를
  낮추면 후보는 늘지만 신뢰도 낮은 소수 평가 기반 예측이 섞인다.
- **세 협업 필터링(사용자 기반·아이템 기반·행렬분해)을 같은 test 세트로 통일 비교한 적은 없다**
  — 각각 다른 시점에 다른 목적(재현율, MAE 등)으로 개별 검증했다. 공정 비교하려면 동일한
  평가 프로토콜(같은 test 세트, 같은 지표)로 다시 돌려야 한다.
- **선호 영화(하트) 저장은 로컬 파일(`favorites.json`) 하나뿐**이라 여러 사람이 동시에 같은
  주소로 접속하면 목록을 공유하게 된다 — 개인 배포/로컬 실행 전제이며, 다중 사용자 배포에는
  맞지 않는다(8-4 절에서도 언급).
